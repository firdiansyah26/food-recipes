...
// Form setup
const { handleSubmit, errors, defineField, isSubmitting, values } = useForm<RecipeFormData>({
...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    ...
    const result = await mutate<{ createRecipe: { id: string, slug: string } }>(
    ...
    router.push(`/admin/recipes/${result.createRecipe.id}`)
  } catch (error: any) {
    toast.add({
      title: 'Error',
  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="mt-1 text-gray-600">Add a new recipe to your collection</p>
      </div>
